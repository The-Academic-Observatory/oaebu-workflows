...
class TestGoogleBooksTelescope(SandboxTestCase):
    ...
    def test_telescope(self):
        ...
        with env.create(), sftp_server.create() as sftp_root:
            ...
            with patch("oaebu_workflows.google_books_telescope.google_books_telescope.pendulum.now") as mock_now:
            ...
            release_ti = dagrun.get_task_instance(task_id="fetch_releases")
            self.assertEqual(release_ti.state, State.SUCCESS)
            release_dicts = release_ti.xcom_pull(task_ids="fetch_releases", include_prior_dates=False)
